...
import com.easemob.chatuidemo.myui.UserInfoActivity;
...
public class ContactlistFragment extends Fragment {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				if (Constant.NEW_FRIENDS_USERNAME.equals(username)) {
					...
					startActivity(new Intent(getActivity(),
							...
							UserInfoActivity.PAMAS, adapter.getItem(position)
									.getUsername()));
				}
				return true;
			}
	...
	public void onDestroy() {
		...
		if (contactInfoSyncListener != null) {
			((DemoHXSDKHelper) HXSDKHelper.getInstance())
					.getUserProfileManager().removeSyncContactInfoListener(
	...
	private void getContactList() {
		contactList.clear();
		// 获取本地好友列表
		Map<String, User> users = ((DemoHXSDKHelper) HXSDKHelper.getInstance())
				.getContactList();
