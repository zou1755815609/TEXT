 ...
 * @下午5:29:27
 */
public class MTextView1 extends TextView {
	private List<RectangleArea> datas;
	...
	public MTextView1(Context context) {
		super(context);
		init(context);
	}

	public MTextView1(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public MTextView1(Context context, AttributeSet attrs, int defStyle) {
	...
	public void init(Context context) {
		...
		datas = new ArrayList<RectangleArea>();
	}
		...
		case MotionEvent.ACTION_UP:
			if (System.currentTimeMillis() - time > 500) {
				...
				for (int i = 0; i < datas.size(); i++) {
					RectangleArea eaArea = datas.get(i);
					boolean b = getIsInArea(eaArea, event.getX(), event.getY());
					if (b) {
						Log.e("isClick",
	...
	private boolean getIsInArea(RectangleArea eaArea, float x, float y) {
		if (x > eaArea.getX1() && x < eaArea.getX2() && y > eaArea.getY1()
				&& y < eaArea.getY2()) {
			return true;
		} else {
			return false;
...
//					if (((SpanObject) ob).source.toString().endsWith("\n")
//							&& j == aContentList.line.size() - 1) {
//						newParagraph = true;
//					}
					if (span instanceof DynamicDrawableSpan) {
						...
						paint.setColor(((ForegroundColorSpan) span)
								.getForegroundColor());
						canvas.drawText(((SpanObject) ob).source.toString(),
								realDrawedWidth, height + aContentList.height
										- mFontMetrics.descent, paint);
						float textLength = paint
								.measureText(((SpanObject) ob).source
										.toString());
						RectangleArea area = new RectangleArea(realDrawedWidth,
								realDrawedWidth + textLength, height
										+ aContentList.height
										- mFontMetrics.descent, height
										+ aContentList.height
										- mFontMetrics.descent
										+ aContentList.height);
						datas.add(area);
						Log.e("RectangleArea", area.toString());
						realDrawedWidth += width;
					} else
					// 做字符串处理
					{
